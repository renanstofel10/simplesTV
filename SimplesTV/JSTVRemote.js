var JSTVRemote = {
	_TV:{
		...
		getApps: function(){
			if(Object.apps)
				return Object.keys(this.startApps);
			else{
				var ret = [];
				for(var i in this.startApps){
					ret.push(i);
				}
				return ret;
			}
		...
		startApp: function(appid, cb){
			console.log("Functionality is not supported");
		},
		closeApp: function(appid, cb){
			console.log("Functionality is not supported");
		},
...
JSTVRemote.LGWebOS = function(ip, port, cb){
	...
	this.keys = {
		...
		Enter: function(cb){if(that.mouseWs)that.mouseWs.send("type:click\nname:CLICK\n\n");},
		Back: function(cb){if(that.mouseWs)that.mouseWs.send("type:button\nname:BACK\n\n");},
		Exit: function(cb){if(that.mouseWs)that.mouseWs.send("type:button\nname:EXIT\n\n");},
	...
	this.startApps = {
		Youtube: function(cb){that.sendRequest("ssap://system.launcher/launch", "request", JSON.stringify({id: "youtube.leanback.v4"}), cb)},
		Netflix: function(cb){that.sendRequest("ssap://system.launcher/launch", "request", JSON.stringify({id: "netflix"}), cb)},
		PrimeVideo: function(cb){that.sendRequest("ssap://system.launcher/launch", "request", JSON.stringify({id: "amazon"}), cb)},
		GloboPlay: function(cb){that.sendRequest("ssap://system.launcher/launch", "request", JSON.stringify({id: "globoplaywebos"}), cb)},
		GlobosatPlay: function(cb){that.sendRequest("ssap://system.launcher/launch", "request", JSON.stringify({id: "globosatplay"}), cb)},
		TelecinePlay: function(cb){that.sendRequest("ssap://system.launcher/launch", "request", JSON.stringify({id: "com.globosat.telecineplay"}), cb);}
	};

	this.closeApps = {
		Youtube: function(cb){that.sendRequest("ssap://system.launcher/close", "request", JSON.stringify({id: "youtube.leanback.v4"}), cb)},
		Netflix: function(cb){that.sendRequest("ssap://system.launcher/close", "request", JSON.stringify({id: "netflix"}), cb)},
		PrimeVideo: function(cb){that.sendRequest("ssap://system.launcher/close", "request", JSON.stringify({id: "amazon"}), cb)},
		GloboPlay: function(cb){that.sendRequest("ssap://system.launcher/close", "request", JSON.stringify({id: "globoplaywebos"}), cb)},
		GlobosatPlay: function(cb){that.sendRequest("ssap://system.launcher/close", "request", JSON.stringify({id: "globosatplay"}), cb)},
		TelecinePlay: function(cb){that.sendRequest("ssap://system.launcher/close", "request", JSON.stringify({id: "com.globosat.telecineplay"}), cb);}
	};
...
JSTVRemote.LGWebOS.prototype.getFeatures = function(){
	return {"sendKey":true, "startApp":true, "closeApp":true, "moveMouse":true, "click":true, "scroll":true, "inputText":true, "takeScreenShot":true};
};
...
JSTVRemote.LGWebOS.prototype.startApp = function(appid, cb){
	if(this.startApps[appid]){
		this.startApps[appid](cb);
	} 
	else if(cb)
		cb("Application " + appid + " not installed");
	};

JSTVRemote.LGWebOS.prototype.closeApp = function(appid, cb){
	if(this.closeApps[appid]){
		this.closeApps[appid](cb);
	} 
	else if(cb)
		cb("Application " + appid + " not installed");
	};	
...
JSTVRemote.LGWebOS.prototype.takeScreenShot = function (cb) {
	alert("AQUI2: " + this.ip + ":" + this.port);
    let uri = 'http://' + this.ip + ':' + this.port + '/roap/api/data?target=screen_image';
    let options = {
        headers: {
    ...
    alert("AQUI4!");
   	var data = JSTVRemote._utils.request(uri, cb, "GET", options);
   	alert("AQUI5!");
   	data.then(stream => stream.pipe(require('fs').createWriteStream('./screenshot.jpg')));
};

//EXPORTS
exports.JSTVRemote = JSTVRemote;